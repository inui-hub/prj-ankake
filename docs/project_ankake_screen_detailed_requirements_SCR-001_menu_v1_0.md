...
画面中央に主要メニューを配置し、CPU対戦を主操作、デッキ構築を副操作として視覚的な優先度を分ける。
...
| AREA-003 | メニュー領域 | CPU対戦ボタンおよびデッキ構築ボタンを表示する |
| AREA-004 | バージョン情報領域 | アプリケーションのバージョン情報を表示する |

CPU対戦ボタンは最も優先度の高い主操作として大きく表示し、デッキ構築ボタンは副操作としてその下に表示する。
...
初期表示処理が完了するまで、CPU対戦ボタンおよびデッキ構築ボタンを操作できない。
...
CPU対戦ボタンおよびデッキ構築ボタンは、以下の表示状態を持つ。
...
CPU対戦ボタンは、通常状態でもデッキ構築ボタンより強く目立つ表現とする。
...
CPU対戦ボタンまたはデッキ構築ボタンの操作時に必要となるデータは、各遷移先画面の詳細仕様で定義する。
...
3. CPU対戦ボタンを300ミリ秒でフェードインしながら、下方向から12ピクセル移動させる。
4. CPU対戦ボタンの表示開始から120ミリ秒後に、デッキ構築ボタンを同様に表示する。
5. バージョン情報を200ミリ秒でフェードインする。
