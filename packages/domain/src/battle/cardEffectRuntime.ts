...
export function resolveCardEffectScript(
  ...
  switch (cardId) {
    ...
    case "AK-011": return damageLaneEnemiesAndBases(state, context, selectedLane(context), 5, firstSequence);
    case "AK-012": return resolveVargas(state, context, firstSequence);
    ...
    // AK-033 is a derived aura.  Costs are evaluated when a hand creature is
    // displayed, validated, or paid, rather than permanently rewriting cards.
    case "AK-033": return withEvent(state, context, firstSequence, "Hand creature cost aura is active.");
    case "AK-034": return state.players[context.controllerSide].maxPp >= 10
      ? drawMatchingDeckCard(state, context, (card) => isCreature(card) && card.cost <= 4, firstSequence, undefined, 0)
      : withEvent(state, context, firstSequence, "Maximum PP condition was not met.");
    ...
    case "AK-044": return summonTokensThenBuff(state, context, "AK-T-001", selectedCoordinates(context), { health: 2 }, firstSequence);
    ...
    case "AK-048": return summonTokensThenBuff(state, context, "AK-T-001", selectedCoordinates(context).slice(0, 3), { attack: 3, health: 3 }, firstSequence);
    ...
    case "AK-022": return drawOncePerTurn(state, context, firstSequence);
...
function selectedBaseId(context: EffectContext) {
  return context.selection.kind === "base" ? context.selection.baseId
    : context.selection.kind === "structured" ? context.selection.value.baseIds?.[0]
    : undefined;
}
...
function damageLaneEnemiesAndBases(state: BattleState, context: EffectContext, lane: ReturnType<typeof selectedLane>, amount: number, sequence: number) {
...
function resolveVargas(state: BattleState, context: EffectContext, sequence: number) {
  ...
  const destroyed = initial.events.find((event) => event.type === "creature.destroyed" && event.instanceId === target.instanceId);
...
function drawOncePerTurn(state: BattleState, context: EffectContext, sequence: number) {
  const source = state.cardInstances[context.sourceInstanceId];
  ...
  const marked: BattleState = { ...state, cardInstances: { ...state.cardInstances, [source.instanceId]: { ...source, effectUsesThisTurn: [...(source.effectUsesThisTurn ?? []), context.effect.effectId] } } };
...
function summonTokensThenBuff(state: BattleState, context: EffectContext, tokenCardId: string, coordinates: readonly { readonly column: number; readonly row: number }[], bonus: { readonly attack?: number; readonly health?: number }, sequence: number) {
...
function reviveRandomAdjacent(state: BattleState, context: EffectContext, sequence: number): { readonly state: BattleState; readonly events: readonly BattleEvent[] } {
  ...
  const player = state.players[context.controllerSide];
  const cards = player.graveyardZone.map((id) => state.cardInstances[id]!).filter((candidate) => isCreature(candidate) && candidate.cost <= 5).sort((a, b) => a.instanceId.localeCompare(b.instanceId));
